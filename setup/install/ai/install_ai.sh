...
if [ -z "${AI_USERS}" ]; then
    printf '[ai] ERROR: AI_USERS is empty and %s names no B2B_LOGIN\n' "$B2B_BUILD_CONF" >&2
    exit 1
fi
...
choose_model() {
    ...
    if [ "$mb" -ge 20480 ]; then
        ...
        echo "qwen3:8b" # ~5 GB
    elif [ "$mb" -ge 5120 ]; then
        echo "qwen3:4b" # ~2.6 GB
    elif [ "$mb" -ge 3072 ]; then
        echo "qwen3:1.7b" # ~1.4 GB
    else
        echo ""
    fi
}
...
install_ollama() {
    ...
    if ! curl -fsSL --max-time 120 --retry 2 -o "${tmp}/ollama.sh" https://ollama.com/install.sh; then
        rm -rf "$tmp"
        warn "could not download the Ollama installer"
        return 1
    fi
    if ! head -n1 "${tmp}/ollama.sh" | grep -q '^#!'; then
        rm -rf "$tmp"
        warn "the downloaded Ollama installer is not a script"
        return 1
    fi
    sh "${tmp}/ollama.sh" >/dev/null 2>&1 || {
        rm -rf "$tmp"
        warn "the Ollama installer failed"
        return 1
    }
    rm -rf "$tmp"
    command -v ollama >/dev/null 2>&1 || {
        warn "ollama is still not on PATH"
        return 1
    }
