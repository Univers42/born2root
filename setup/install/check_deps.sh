...
# /dev/kvm and nothing else, and the run died inside an interactive
...
vbox_required() {
    ...
    [ -r "$KVM_DEV" ] && [ -w "$KVM_DEV" ] && return 1
    return 0
}

# The vboxusers group is worth offering only for a VirtualBox that is both
# installed and going to be used.
vbox_group_wanted() {
    [ "$VBOX_OK" = true ] || return 1
...
check_vbox() {
    if command -v VBoxManage >/dev/null 2>&1; then
        ...
        printf '%s·%s VirtualBox not installed %s(not needed: QEMU/KVM runs this build)%s\n' \
            "$DIM" "$RST" "$DIM" "$RST"
    fi
}
