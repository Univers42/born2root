...
install_for_user() {
    ...
    [ -f "${SRC}/.hellishrc" ] ||
        {
            warn "${user}: source has no .hellishrc — nothing would load"
            return 1
        }
...
verify_for_user() {
    ...
    *)
        warn "${user}: verify reported problems:"
        printf '%s\n' "$out" | sed 's/^/[hellishrc]   /'
        ;;
    esac
}
