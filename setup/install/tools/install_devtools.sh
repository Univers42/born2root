...
case "$(uname -m)" in
	...
	*) warn "unsupported architecture $(uname -m) — skipping Herdr"; INSTALL_HERDR=0; ARCH="" ;;
esac
...
install_herdr() {
	...
	install -m 755 "${tmp}/herdr" "$HERDR_DEST" || { warn "install failed"; rm -rf "$tmp"; return 0; }
	rm -rf "$tmp"
...
# ── Claude Code ─────────────────────────────────────────────────────────────
install_claude_code() {
	command -v npm >/dev/null 2>&1 || { warn "npm not installed — skipping Claude Code"; return 0; }
	...
	if npm install -g @anthropic-ai/claude-code >/dev/null 2>&1; then
		...
		local prefix; prefix=$(npm config get prefix --global 2>/dev/null)
...
setup_herdr_service() {
	...
	command -v systemctl >/dev/null 2>&1 || { warn "no systemd — skipping the herdr service"; return 0; }

	local user home group
	for user in $DEVTOOLS_USERS; do
		home=$(getent passwd "$user" 2>/dev/null | cut -d: -f6)
		[ -n "$home" ] && [ -d "$home" ] || { warn "user '${user}' has no home — skipping"; continue; }
