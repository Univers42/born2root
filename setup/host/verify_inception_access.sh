...
if [ -n "$P_HTTPS" ]; then
    pass "https  host:${P_HTTPS} → guest:443"
else
    fail "no 'https' NAT rule"
fi
if [ -n "$P_STATIC" ]; then
    pass "static host:${P_STATIC} → guest:8090"
else
    fail "no 'inception-static' NAT rule"
fi
: "${P_HTTPS:=8443}"
: "${P_STATIC:=8090}"
...
if [ "$code" = "200" ]; then
    pass "http://${DOMAIN}:${P_STATIC}/ → 200 (bonus static site)"
else
...
if [ -s "$ca_tmp" ]; then
    ...
    *) fail "https://${DOMAIN}/ → ${res%%:*} through the proxy" ;;
    ...
    if [ "$res" = "200" ]; then
        pass "https://${DOMAIN}/ → 200 through the proxy"
    else
...
if [ "$other" = "403" ]; then
    pass "proxy refuses every other host (example.com → 403)"
else
