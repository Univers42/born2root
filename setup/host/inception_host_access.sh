...
set -uo pipefail
# The interpreter written into the two helpers this script generates on the
# host (the browser launcher, the curl wrapper): the shell make picked, else
# the one running this very script -- as an absolute path, since a desktop
# launcher's PATH need not include ~/.local/bin.
HOST_SH="${SCRIPT_SH:-$(readlink /proc/$$/exe 2>/dev/null || command -v bash)}"
...
if have_vm; then
    ...
    P_HTTPS="$PREF_HTTPS_PORT"
    P_STATIC="$PREF_STATIC_PORT"
    P_HTTP="$PREF_HTTP_PORT"
