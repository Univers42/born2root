...
The Docker stack inside the VM was healthy: `https://localhost:4322` returned
HTTP 200 from inside `b2b`.

Firefox on the host could not connect because `localhost:4322` on the host is
not the same network namespace as `localhost:4322` inside the VM. With
VirtualBox NAT, every host-visible port needs a NAT forwarding rule.

The VM had forwarding for the older development ports, but it did not have the
osionos / ft_transcendence ports:
...
The important Docker line is `local-https-proxy`; it should publish these VM
ports:
...
VirtualBox NAT rules live in the VM configuration. You can add, delete, or
repair them from the host with `VBoxManage` while the VM is running. A rebuild
is only needed when the guest OS itself is broken.

The automation now applies these rules during `make all`, and
`make fix_app_ports` repairs an already-created VM.
