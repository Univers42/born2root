...
# ── Nerd Font icons in the guest's Neovim, decided before any download ─────
# Everything in the editor keys off kickstart's vim.g.have_nerd_font, which
# ships false -- and render-markdown then "renders" a heading as the icon `## `
# and a checkbox as `[ ] `, so a rendered buffer looks almost exactly like the
# raw file (reported 2026-09-13). The glyphs are drawn by the terminal on the
# machine you ssh FROM, so the build asks that machine: a Nerd Font in
# fontconfig means on. It cannot tell whether your terminal uses it, hence
# NVIM_NERD_FONT to say so.
#
# NVIM_NERD_FONT, not NERD_FONT. The first version read NERD_FONT, and the
# owner's ~/.hellishrc has `export NERD_FONT=0` for its prompt theme: every
# build from that shell died with "NERD_FONT must be auto, on or off (got
# '0')" -- after the ISO download, and after `make re` had already deleted the
# old VM. A generic name belongs to whoever set it first; this one is
# namespaced like NVIM_VERSION and NVIM_USERS, and is the same variable
# install_nvim.sh reads in the guest. It is resolved here, before anything is
# fetched, and 0/1, true/false and yes/no are accepted as well as on/off.
resolve_nerd_font() {
    case "${1:-auto}" in
    on | ON | On | 1 | true | TRUE | True | yes | YES | Yes) printf 'on' ;;
    off | OFF | Off | 0 | false | FALSE | False | no | NO | No) printf 'off' ;;
    auto | AUTO | Auto)
...
if ! NERD_FONT_RESOLVED=$(resolve_nerd_font "${NVIM_NERD_FONT:-auto}"); then
    echo "Error: NVIM_NERD_FONT must be auto, on or off (got '${NVIM_NERD_FONT}') — nothing was downloaded" >&2
    exit 1
fi
