...
emit_recipe() {
	...
	printf '\t. \\\n'
	s=$(size_of root);    recipe_stanza root    "$s" "$s" "$s" ext4 /;        printf '\t. \\\n'
	s=$(size_of swap);    recipe_stanza swap    "$s" "$s" "$s" linux-swap ''; printf '\t. \\\n'
	s=$(size_of home);    recipe_stanza home    "$s" "$s" "$s" ext4 /home;    printf '\t. \\\n'
	s=$(size_of opt);     recipe_stanza opt     "$s" "$s" "$s" ext4 /opt;     printf '\t. \\\n'
	s=$(size_of srv);     recipe_stanza srv     "$s" "$s" "$s" ext4 /srv;     printf '\t. \\\n'
	s=$(size_of tmp);     recipe_stanza tmp     "$s" "$s" "$s" ext4 /tmp;     printf '\t. \\\n'
	s=$(size_of var-log); recipe_stanza var-log "$s" "$s" "$s" ext4 /var/log; printf '\t. \\\n'
