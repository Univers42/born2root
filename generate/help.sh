...
sec "Make it yours: born2root.conf (repo root)"
...
cmd "VM_NAME=…" "Which VM to act on (default: born2root.conf)" "${DIM}"
cmd "VM_PASS=…" "LUKS passphrase (default: born2root.conf)" "${DIM}"
cmd "HELLISH_VERSION=…" "Pin a hellish release (default: latest)" "${DIM}"
cmd "B2B_CONFIG=…" "Read another born2root.conf" "${DIM}"
...
cmd "SIZE_B2B=…" "Whole VM disk in GB (born2root.conf: 15, min 8)" "${DIM}"
...
blank
row "    ${DIM}settings given here beat born2root.conf for this run${RST}"
