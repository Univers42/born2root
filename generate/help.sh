...
cmd "make nvim" "Neovim + kickstart + extras + Excalidraw, plugins verified"
...
cmd "make devtools" "Herdr (persistent panes) + opencode"
cmd "make claude_code" "Claude Code, beside opencode (320 MB on /)"
note "auto from SIZE_B2B=30; below that FEATURES=\"+claude-code\""
cmd "make llm_host" "Local models for opencode: llama.cpp on this host's GPU"
cmd "make llm_select" "Pick the models (from Hugging Face) and their folder"
note "[ai] in born2root.toml; make llm_status / llm_stop"
cmd "make features_select" "Tick what goes in the VM; the disk grows to fit"
note "make all runs it when a terminal is attached; --clear forgets it"
...
cmd "VM_NAME=…" "Which VM to act on (default: born2root.toml)" "${DIM}"
cmd "VM_PASS=…" "LUKS passphrase (default: born2root.toml)" "${DIM}"
cmd "HELLISH_VERSION=…" "Pin a hellish release (default: latest)" "${DIM}"
cmd "B2B_CONFIG=…" "Read another born2root.toml" "${DIM}"
...
cmd "SIZE_B2B=…" "Whole VM disk in GB (born2root.toml: 15, min 8)" "${DIM}"
cmd "PROFILE=…" "auto | minimal | standard | full (from SIZE_B2B)" "${DIM}"
...
blank
row "    ${DIM}settings given here beat born2root.toml for this run${RST}"
