...
*)
    printf 'FAIL %-38s = %s\n' "permission denied: why" "$(vboxdrv_why)"
    fail=1
    ;;
esac
...
hardened="$TMP/lib-hardened"
mkdir -p "$hardened"
printf '#!/bin/sh\n' >"$hardened/VirtualBoxVM"
chmod 4711 "$hardened/VirtualBoxVM"

developer="$TMP/lib-developer"
mkdir -p "$developer"
printf '#!/bin/sh\n' >"$developer/VirtualBoxVM"
chmod 0755 "$developer/VirtualBoxVM"
